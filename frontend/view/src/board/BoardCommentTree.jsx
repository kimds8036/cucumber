...
export default function BoardCommentTree({
  ...
  const renderComment = useMemo(
    () => (item, isReply = false, parentAuthorLabel = null) => {
      const isCommentLiked = commentLikedState[item.id] !== undefined
          ...
          <View style={styles.commentFooter}>
            ...
            >
              <TouchableOpacity
                ...
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                onPress={() => onOpenMenu(item.id, commentMenuRefs.current[item.id])}
              >
      ...
      if (isReply) {
        return (
          <View key={item.id} style={styles.commentItemReply} collapsable={false}>
    ...
    [commentLikedState, commentMenuRefs, normalize, onCommentLike, onFocusReply, onOpenMenu, replyToCommentId, styles]
  );

  const renderItem = ({ item }) => {
    if (item.type === 'comment') {
      return <View style={{ paddingHorizontal: horizontalPadding }}>{renderComment(item.data, false, null)}</View>;
    }
    if (item.type === 'reply') {
      return (
