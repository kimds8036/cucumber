...
const CATEGORY_PUBLIC = ['login_error', 'account_suspension'];
const CATEGORY_AUTH = ['bug', 'feedback'];

const CATEGORY_LABELS = {
  login_error: '로그인 오류',
  account_suspension: '계정 정지 문의',
  bug: '버그 신고',
  feedback: '제안 / 피드백',
};

const CATEGORY_HINTS = {
  login_error: '로그인이 안 되거나 비밀번호 재설정이 안 되는 등의 문제를 알려주세요.',
  account_suspension: '계정 정지 사유에 대한 이의나 해제 요청을 알려주세요.',
  bug: '발생한 버그를 가능한 자세히 적어주세요. 재현 절차, 화면, 발생 시각이 도움이 됩니다.',
  feedback: '서비스 개선 아이디어나 불편한 점을 자유롭게 작성해주세요.',
};
...
const Inquiry = ({ navigation, route }) => {
  ...
  const initialCategory = String(route?.params?.category || '').trim();
  const initialContactUsername = String(route?.params?.contactUsername || '').trim();

  const allowedCategories = useMemo(
    () => (isLoggedIn ? CATEGORY_AUTH : CATEGORY_PUBLIC),
    [isLoggedIn]
  );

  const [category, setCategory] = useState(() => {
    if (initialCategory && allowedCategories.includes(initialCategory)) return initialCategory;
  ...
  const handlePickImages = async () => {
    if (images.length >= 5) {
      Alert.alert('알림', '이미지는 최대 5장까지 첨부할 수 있어요.');
      return;
    }
    try {
  ...
  const handleSubmit = async () => {
    ...
    if (!isLoggedIn) {
      if (!contactUsername.trim()) {
        Alert.alert('알림', '본인 확인을 위해 아이디를 입력해주세요.');
        return;
      }
      if (!contactPhone.trim()) {
        Alert.alert('알림', '본인 확인을 위해 전화번호를 입력해주세요.');
        return;
      }
    }
    setSubmitting(true);
    try {
      ...
      if (contactUsername.trim()) formData.append('contact_username', contactUsername.trim());
  ...
  const showContactFields = !isLoggedIn;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        ...
        >
          <View style={[styles.labelRow, styles.labelRowFirst]}>
            <Text style={styles.sectionLabelText}>카테고리</Text>
          </View>
          ...
          <FieldLabel text="제목" required styles={styles} />
          <TextInput
            style={styles.input}
            value={title}
          ...
          {showContactFields ? (
            <>
              <FieldLabel text="아이디" required styles={styles} />
              <TextInput
                style={styles.input}
                ...
                maxLength={50}
              />
              <FieldLabel text="전화번호" required styles={styles} />
              <TextInput
                style={styles.input}
                value={contactPhone}
