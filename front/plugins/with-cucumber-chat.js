  ...
  return withXcodeProject(config, (config) => {
    ...
    for (const file of files) {
      ...
      if (result === 'addedByAddSourceFile') {
        ...
        String(result).startsWith('addSourceFileError:already there')
      ) {
        skippedCount += 1;
      } else {
        failedCount += 1;
    ...
    if (failureSamples.length > 0) {
      console.log(
        `[with-cucumber-chat] xcode failure samples: ${failureSamples.join(' | ')}`,
      );
    }
...
function withCucumberChatPod(config) {
  return withPodfile(config, (config) => {
    ...
    if (!config.modResults.contents.includes('Socket.IO-Client-Swift')) {
      ...
      console.log(
        '[with-cucumber-chat] Podfile updated with Socket.IO-Client-Swift',
      );
    } else {
      console.log(
        '[with-cucumber-chat] Podfile already contains Socket.IO-Client-Swift',
      );
    }
    return config;
  });
