  ...
  const TableHeader = ({ cols }) => (
    <View style={[s.tableRow, s.tableHeaderRow]}>
      {cols.map((col, i) => (
        <Text
          key={i}
          style={[s.tableCell, s.tableCellHeader, i === 0 && s.tableCellFirst]}
        >
          {col}
        </Text>
      ))}
  ...
  return (
    ...
    >
      <SafeAreaView style={s.container} edges={['top', 'bottom']}>
        ...
        >
          <Para>
            [상호명](이하 "회사")은 「Cucumber」 모바일 애플리케이션 서비스(이하
            "서비스")와 관련하여 정보주체의 개인정보를 중요하게 여기며,
            「개인정보 보호법」, 「정보통신망 이용촉진 및 정보보호 등에 관한
            법률」 등 관련 법령을 준수합니다.
          </Para>
          <Para>
            본 처리방침은 법령의 변경 또는 회사 정책에 따라 변경될 수 있으며,
            변경 시 서비스 내 공지 등을 통해 안내합니다.
          </Para>
          ...
          <View style={s.table}>
            ...
            <TableRow
              cells={[
                '계정·본인 확인',
                '사용자명(아이디), 비밀번호(암호화 저장), 이름, 휴대전화번호, 생년월일',
              ]}
            />
            <TableRow
              cells={[
                '학교 소속',
                '학교명 등 학교 식별 정보, 학년, 반, 졸업예정연도',
              ]}
            />
            ...
            <TableRow
              cells={[
                '가입·인증 과정',
                '전화번호 인증을 위한 인증번호 발송·검증 과정에서의 휴대전화번호, 인증 시각 등',
              ]}
            />
          </View>
          ...
          <View style={s.table}>
            ...
            <TableRow
              cells={[
                '로그인·보안',
                '단말 식별자, 단말 정보(User-Agent 등), IP 주소, 최종 로그인 시각',
              ]}
            />
            <TableRow
              cells={[
                '게시판·커뮤니티',
                '게시글·댓글·좋아요·스크랩·해시태그, 게시글 작성 시 선택적으로 수집되는 위도·경도',
              ]}
            />
            <TableRow
              cells={[
                '이미지',
                '게시글·댓글·쪽지·DM 등에 첨부하는 사진·이미지 파일',
              ]}
            />
            <TableRow
              cells={[
                '쪽지·DM',
                '대화방·메시지 내용, 읽음 여부, 삭제·복구 관련 상태값',
              ]}
            />
            ...
            <TableRow
              cells={[
                '타이머·학습',
                '일별 공부 시간, 과목·메모 등 이용자가 입력한 데이터',
              ]}
            />
            <TableRow
              cells={['학생 인증(선택)', '학생증 이미지 URL, OCR 추출 JSON 등']}
            />
          </View>

          <Text style={s.subTitle}>3. 자동 수집 장치</Text>
          <Bullet>
            서비스 이용 과정에서 OS·앱 버전, 단말 식별자, IP, 이용 기록 등이
            자동 생성되어 수집·저장될 수 있습니다.
          </Bullet>
          <Bullet>
            향후 분석 SDK·광고 SDK를 도입하는 경우 별도 동의 및 본 방침 개정을
            통해 안내합니다.
          </Bullet>
          ...
          <Bullet>
            회원 가입·의사 확인, 본인 확인, 중복 가입 방지, 만 14세 미만 여부
            판단 및 법정대리인 동의 절차
          </Bullet>
          <Bullet>
            서비스 제공: 게시판, 댓글, 쪽지·DM, 우편, 검색, 친구, 알림, 타이머,
            급식·시간표 연동 등
          </Bullet>
          ...
          <Bullet>
            파기 방법: 전자적 파일은 복구 불가능한 방법으로 삭제, 출력물은 분쇄
            또는 소각
          </Bullet>
          ...
          <Para>
            ...
            다만 정보주체가 사전에 동의한 경우, 법령에 특별한 규정이 있거나
            수사·감독 목적의 적법한 요청이 있는 경우는 예외로 합니다.
          </Para>
          ...
          <View style={s.table}>
            ...
            <TableRow
              cells={['클라우드·호스팅 사업자', '데이터 저장, 서버 운영']}
            />
            <TableRow cells={['이미지 저장(CDN 등)', '첨부 이미지 호스팅']} />
            <TableRow
              cells={['문자·알림 발송 대행업체', '전화번호 인증 문자 발송']}
            />
          </View>
          ...
          <Para>
            정보주체는 언제든지 개인정보 열람 요구, 오류 등이 있을 경우
            정정·삭제 요구, 처리 정지 요구의 권리를 행사할 수 있습니다. 권리
            행사는 서비스 내 설정, 고객센터 이메일 등을 통해 가능하며, 회사는
            지체 없이 조치합니다.
          </Para>
          <Para>
            만 14세 미만 아동의 경우 법정대리인이 권리를 행사할 수 있으며,
            회사는 법령에 따라 법정대리인 동의 확인 절차를 둘 수 있습니다.
          </Para>
          ...
          <Bullet>
            기술적 조치: 비밀번호 암호화 저장, 접근 통제, 전송 구간 보안(HTTPS
            등)
          </Bullet>
          ...
          <View style={s.infoBox}>
            <Text style={s.infoRow}>
              <Text style={s.infoLabel}>성명</Text> 김은채
            </Text>
            <Text style={s.infoRow}>
              <Text style={s.infoLabel}>직책</Text> 서비스 운영 담당
            </Text>
            <Text style={s.infoRow}>
              <Text style={s.infoLabel}>이메일</Text> eunchae6589@gmail.com
            </Text>
          </View>
